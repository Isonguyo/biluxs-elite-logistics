          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {list.map((v, i) => (
              <motion.div key={v.id}
                ...
                <TiltCard className="bg-card border border-border overflow-hidden group hover:border-gold transition-colors will-change-transform">
                  <div className="aspect-video bg-[var(--navy-deep)] grid place-items-center relative overflow-hidden">
                    {v.image_url ? (
                      <ParallaxY distance={30} className="h-full w-full">
                        <img src={v.image_url} alt={v.name} className="h-full w-full object-cover scale-110 group-hover:scale-125 transition-transform duration-700" />
                      </ParallaxY>
                    ) : <Car className="h-16 w-16 text-muted-foreground/40" />}
                    <div className={`absolute top-3 right-3 px-2 py-1 text-[10px] uppercase tracking-widest z-10 ${
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-4 flex items-end justify-between">
                      ...
                      <Link to="/book" search={{ vehicle: v.id }}
                        className="px-4 h-10 bg-crimson text-white text-[10px] uppercase tracking-widest press-effect inline-flex items-center liquid-glow">
                        Book
                      </Link>
                    </div>
                  </div>
